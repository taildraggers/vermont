...
Daily aggregator of Vermont-located taildragger classified listings,
...
[Utah](https://github.com/taildraggers/utah) repos' approach, with the
state filter swapped to "VT".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, VT` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Vermont"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/vermont/`
...
```html
<iframe
  src="https://taildraggers.github.io/vermont/"
  title="Taildragger Ads in Vermont"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_VERMONT_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
