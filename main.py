"""Aggregates Vermont-located listings from every taildraggers.com
...
ones whose location ends in ", VT".
...
repos' approach, with the state filter swapped to "VT".
"""
...
PAGE_TITLE = "Taildragger Ads in Vermont"
STATE_ABBR = "VT"
...
_VERMONT_LOCATION_RE = re.compile(r",\s*VT\s*$", re.IGNORECASE)


def _is_vermont(location: str) -> bool:
    return bool(_VERMONT_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        vermont_rows = [row for row in rows if _is_vermont(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(vermont_rows)} in Vermont")
        all_listings.extend(vermont_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Vermont listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Vermont listings: {len(listings)}")
